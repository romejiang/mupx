...
exports.setup = function(config) {
  ...
  taskList.copy('Copying app run shell', {
    src: path.resolve(SCRIPT_DIR, 'run_app.sh'),
    dest: '/opt/run_app.sh'
  });

  taskList.copy('Copying fibers packages', {
    src: path.resolve(TEMPLATES_DIR, 'fibers-1.0.10.tar.gz'),
    dest: '/opt/fibers-1.0.10.tar.gz'
  });
...
exports.verify = function(config) {
  var taskList = nodemiral.taskList("Verify Application (linux)");

  var appName = config.appName;
  var port = config.env.PORT;
  var deployCheckWaitTime = config.deployCheckWaitTime;

  verify(taskList, appName, port, deployCheckWaitTime);
  return taskList;
};
